...
class SessionManager:
    """Record a live session and persist its summary to disk."""
    ...
    def end_session(self) -> dict | None:
        """Stop recording, compute summary, persist, and return summary."""
    ...
    def _persist(self, summary: dict) -> None:
        """Append summary to the JSON history file."""
        existing: list[dict] = []
...
def generate_tips(summary: dict) -> list[str]:
    """Return actionable improvement tips based on summary stats."""
    tips: list[str] = []
    ...
    if avg < 60:
        tips.append(
            "Your overall confidence was low. "
            "Practice speaking at a steady pace with fewer pauses."
        )
    if avg >= 80:
        tips.append(
            "Excellent overall confidence. "
            "Keep maintaining this level in real interviews."
        )
    if low > 30:
        tips.append(
            "You spent significant time below 50% confidence. "
            "Work on reducing long silences and hesitation."
        )
    if high < 40:
        tips.append(
            "Less than 40% of your session was high-confidence. "
            "Focus on projecting certainty in your answers."
        )
    elif high > 70:
        tips.append(
            "Great job — over 70% of your session was in the "
            "high-confidence zone."
        )

    if not tips:
        tips.append(
            "Solid session. Continue practising to maintain "
